...
MAX_AGE_DAYS = 4  # ✅ ИЗМЕНЕНО: 4 дня (запас для запуска раз в 3 дня)
MAX_POSTS_PER_RUN = 100  # ✅ ИЗМЕНЕНО: 100 постов (за 3 дня накапливается больше)
GROQ_DELAY = 2
MESSAGE_DELAY = 3  # Telegram безопасный интервал (20 сообщений/минуту)
MIN_STARS = 1
...
def is_repo_empty(owner, repo, cache):
    ...
    try:
        ...
        if resp.status_code != 200:
            ...
            # Проверяем размер и активность
            result = (
                data.get('size', 0) < 5 or  # Меньше 5 KB
...
def is_likely_fork_spam(item):
    ...
    # Форк без звёзд и активности
    if item.get('stargazers_count', 0) == 0 and item.get('forks_count', 0) == 0:
        return True
    
    # Форк без изменений (совпадает дата создания и последнего пуша)
...
def safe_desc(desc, max_len=120):
    ...
    desc = str(desc).strip()
    desc = re.sub(r'[🔥⚡️✨🎉]{3,}', '', desc)  # Удаляем эмодзи-спам
...
def quick_filter(name, desc, stars=0):
    ...
    noise_patterns = [
        ...
        r'[A-Z]{8,}',  # Много заглавных букв
        r'[-_]{3,}',  # Много дефисов/подчёркиваний
    ]
...
def search_fresh_repos(query, per_page=30):  # ✅ ИЗМЕНЕНО: 30 вместо 20
...
async def generate_desc(name, desc):
    """✅ Генерация описания через AI (с retry)"""
...
Описание:"""
    ...
    for attempt in range(2):  # 2 попытки
        try:
            ...
            return generated if generated else (desc or "Инструмент для обхода блокировок")
        except Exception as e:
            logger.debug(f"AI description attempt {attempt+1} failed: {e}")
            await asyncio.sleep(1)
    
    return desc or "Инструмент для обхода блокировок"
...
async def main():
    ...
    for agg in KNOWN_AGGREGATORS:
        ...
        if not c:
            logger.debug(f"   ⏭ SKIP: {agg['name']} (error or non-Latin)")
            continue
    ...
    for s in FRESH_SEARCHES:
        ...
        # Проверяем лимиты перед каждым поиском
        if not check_rate_limit():
